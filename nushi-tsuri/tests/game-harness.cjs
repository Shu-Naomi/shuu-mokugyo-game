...
const html = fs.readFileSync(path.join(__dirname, "../index.html"), "utf8")
  .replace(/<script src="((?:ui-feedback|event-ceremony|fortune-capsules|aquarium-life|pet-life(?:-ui)?|tackle-(?:balance|art)|coast-voyage|pixel-(?:world|cast)|weather|rival-anglers|tournament(?:-npcs)?|music-tracks|soundscape|scene-layers|layered-scenery)\.js)\?[^\"]+"><\/script>/g,
    (_, name) => `<script>${fs.readFileSync(path.join(__dirname, "..", name), "utf8")}</script>`)
  .replace(/<link rel="stylesheet" href="((?:event-ceremony|fortune-capsules|pixel-scenes|pet-life)\.css)\?[^\"]+" \/>/g,
